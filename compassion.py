...
def alpha_gradient_plot(bag,gram,total_times,mini_size):
    ...
    for alpha in alphas:
        soft=Softmax(len(bag.train),5,bag.len)
        soft.regression(bag.train_matrix,bag.train_y,alpha,total_times//mini_size,'shuffle')
        r_train, r_test = soft.correct_rate(bag.train_matrix, bag.train_y, bag.test_matrix, bag.test_y)
    ...
    plt.semilogx(alphas,batch_train,'b-',label='batch')
    plt.semilogx(alphas,mini_train,'g-',label='mini_batch')
    plt.semilogx(alphas,shuffle_train,'ro-',alphas,batch_train,'g+-',alphas,mini_train,'b^-')
    plt.legend()
    plt.title('Training set of Bag of words ')
    ...
    plt.semilogx(alphas,batch_test,'b--',label='batch')
    plt.semilogx(alphas,mini_test,'g--',label='mini_batch')
    plt.semilogx(alphas,shuffle_test,'ro-',alphas,batch_test,'g+-',alphas,mini_test,'b^-')
    plt.legend()
    plt.title('Testing set of Bag of words ')
    plt.xlabel('Learning rate')
    plt.ylabel('Accuracy')
    plt.ylim(0,1)
